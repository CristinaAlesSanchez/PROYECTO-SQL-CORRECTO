...
select f.rental_rate as "Costo" , f.rental_duration  as "Dia" 
from film f 
...
select round(AVG(f.length ),2) as "Promedio duracion" 
from film f ;
...
select AVG(r.rental_date - r.return_date ) as "Media duracion"
from rental r 
...
select r.rental_date as "Dia" , 
        count  (r.rental_date ) as "Cantidad Alquiler"
from rental r
...
from film f 
join (
       select AVG(f.rental_rate) as "Precio medio"
       from film f )
on f.rental_rate > "Precio medio" 
...
--No, mezcla todas las peliculas con las categorias sin orden ninguno
...
join inventory i on f.film_id = i.film_id 
join rental r on i.inventory_id = r.inventory_id 
where r.return_date > DATE_ADD(r.rental_date , interval '8' day )
order by f.title ;
...
select s.staff_id ,
      concat(s.first_name ,' ', s.last_name ) as "Trabajadores" , s.store_id 
from staff s 
cross join store s2 ;
